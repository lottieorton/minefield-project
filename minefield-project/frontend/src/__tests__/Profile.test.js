...
describe("fetches user info", () => {
  ...
  it("passes updated user info to component to render", async () => {
    ...
    const firstName = await screen.findByText(
      `First Name: ${testUser.user.first_name}`,
    );
    const lastName = await screen.findByText(
      `Last Name: ${testUser.user.last_name}`,
    );
...
describe("handleSubmit", () => {
  it("handleSubmit makes correct API call with payload values", async () => {
    ...
    global.fetch = jest
      ...
      .mockResolvedValueOnce({
        ...
        status: 200,
        json: async () => ({
          user: { username: "testUser", first_name: "Test", last_name: "Name" },
        }),
      });
  ...
  it("renders success message if response.ok and form fields back to blank", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/success! User details updated for testUser./i),
      ).toBeInTheDocument();
