...
jest.mock("../components/presentational/ScoreCard.js", () => {
  return function MockedScoreCard(props) {
    ...
    return (
      <div
        data-testid={"mock-game-card" + game.difficulty}
        className="score-card"
        key={game.id + game.difficulty}
      >
        {game.difficulty}: Wins: {game.wins}. Losses: {game.losses}. W/L Ratio:{" "}
        {game.wlratio}%
      </div>
    );
  };
...
describe("fetches users score", () => {
  ...
  it("passes updated game scores via prop to ScoreCards", async () => {
    ...
    await waitFor(() => {
      ...
      expect(easyCard).toHaveTextContent(
        `${finalScores[0].difficulty}: Wins: ${finalScores[0].wins}. Losses: ${finalScores[0].losses}. W/L Ratio: ${finalScores[0].wlratio}%`,
      );
      expect(mediumCard).toHaveTextContent(
        `${finalScores[1].difficulty}: Wins: ${finalScores[1].wins}. Losses: ${finalScores[1].losses}. W/L Ratio: ${finalScores[1].wlratio}%`,
      );
      expect(hardCard).toHaveTextContent(
        `${finalScores[2].difficulty}: Wins: ${finalScores[2].wins}. Losses: ${finalScores[2].losses}. W/L Ratio: ${finalScores[2].wlratio}%`,
      );
    });
  });
