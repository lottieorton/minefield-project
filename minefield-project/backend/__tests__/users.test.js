...
const {
  ensureAuthenticated,
  checkLoggedIn,
} = require("../functions/ensureAuthenticated.js");
  ...
  it("on successful addition of user to database", async () => {
    ...
    expect(db.pool.query).toHaveBeenCalledWith(
      ...
      [
        mockDbUser.username,
        mockDbUser.password,
        mockDbUser.first_name,
        mockDbUser.last_name,
      ],
    );
...
describe("users routes", () => {
  ...
  it("POST /user/register route, which doesnt meet the checks", async () => {
    ...
    expect(response.body).toMatchObject({
      msg: "Password must be at least 8 characters long | Password must contain at least one uppercase letter",
    });
  });
  ...
  it("PATCH /user/updateUser route", async () => {
    ...
    ensureAuthenticated.mockImplementation((req, res, next) => next());
    //action
    const response = await request(app).patch("/user/updateUser").send({
      username: "test",
      first_name: "Lotts",
      last_name: "Here",
    });
  ...
  it("GET /user/loggedInStatus route", async () => {
    ...
    //action
    const response = await request(app).get("/user/loggedInStatus");
    //assert
    expect(response.status).toBe(200);
